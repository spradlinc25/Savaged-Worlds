...
function computeSkillMods() {
  // Returns {skillName: [{label, value}]} from edges, hindrances, powers
  const mods = {};
  function addMod(skill, label, val) {
    skill = skill.toLowerCase();
    if (!mods[skill]) mods[skill] = [];
    mods[skill].push({label, val});
  }
  // Hindrances with skill penalties
  // Collect active hindrances from both bolohad and Starting tab
  const activeHindrances = [
    ...state.hindrances.filter(h=>isTrue(f(h,'acquired','bolohad','bolo_had','bolo'))),
    ...state.starting.filter(s=>s.on && (f(s,'type')||'').toLowerCase().includes('hindrance'))
      .map(s=>{
        const nm=f(s,'name')||'';
        const lib=state.hindrances.find(h=>(h.name||'').toLowerCase().trim()===nm.toLowerCase().trim());
        return lib||{name:nm,effect:f(s,'effect')||''};
      })
  ];
  // deduplicate
  const seenH=new Set();
  const dedupH=activeHindrances.filter(h=>{const n=(h.name||'').toLowerCase();if(seenH.has(n))return false;seenH.add(n);return true;});
  dedupH.forEach(h=>{
    const eff = (f(h,'effect')||'').toLowerCase();
    const nm = f(h,'name')||'';
    // Parse patterns like "-2 persuasion" or "persuasion -2"
    const skillPenaltyRe = /([+-]?\d+)\s+to\s+([\w\s]+?)(?:\s+rolls?)?(?:\.|$|,)/gi;
    let m;
    while ((m = skillPenaltyRe.exec(eff)) !== null) {
      const val = parseInt(m[1]);
      const sk = m[2].trim();
      addMod(sk, `${nm} (${val>=0?'+':''}${val})`, val);
    }
    // Outsider: -2 Persuasion
    if (nm.toLowerCase().includes('outsider') && eff.includes('persuasion')) addMod('persuasion', 'Outsider (-2)', -2);
    if (nm.toLowerCase().includes('all thumbs')) addMod('electronics', 'All Thumbs (-2)', -2);
    if (nm.toLowerCase().includes('anemic')) addMod('vigor', 'Anemic (-2 vs Fatigue)', -2);
    if (nm.toLowerCase().includes('bad eyes') && !nm.toLowerCase().includes('imp')) { addMod('shooting', 'Bad Eyes (-2)', -2); addMod('athletics', 'Bad Eyes thrown (-2)', -2); }
    if (nm.toLowerCase().includes('clumsy')) { addMod('athletics', 'Clumsy (-2)', -2); addMod('stealth', 'Clumsy (-2)', -2); }
    if (nm.toLowerCase().includes('hard of hearing')) addMod('notice', 'Hard of Hearing (-4 audio)', -4);
    if (nm.toLowerCase().includes('yellow')) addMod('spirit', 'Yellow (-2 Fear)', -2);
  });
  // Starting edges with modifiers — check both starting and progression sources
  const allActiveEdges = [
    ...state.starting.filter(s=>s.on).map(s=>({nm:(f(s,'name')||'').toLowerCase(), eff:(f(s,'effect')||'').toLowerCase()})),
    ...state.progressions.filter(p=>p.checked && f(p,'type')==='Edge').map(p=>({nm:(f(p,'selection','name')||'').toLowerCase(), eff:(f(p,'effect')||'').toLowerCase()})),
  ];

  // Use hasE to check for specific edges to safely override upgrades without looping
  const edgeNames = allActiveEdges.map(e => e.nm);
  const hasE = (str) => edgeNames.some(n => n.includes(str));

  // ── Awareness / perception ──────────────────────────────
  if (hasE('alertness'))     addMod('notice',      'Alertness (+2)', 2);
  if (hasE('danger sense'))  addMod('notice',      'Danger Sense (+2 ambush)', 2);
  if (hasE('infravision'))   addMod('notice',      'Infravision (halves dark penalty)', 0);
  
  // ── Combat — Fighting ───────────────────────────────────
  // Martial Warrior supersedes Martial Artist — don't double-stack
  if (hasE('martial warrior')) {
    addMod('fighting', 'Martial Warrior (+2 unarmed)', 2);
  } else if (hasE('martial artist')) {
    addMod('fighting', 'Martial Artist (+1 unarmed)', 1);
  }
  if (hasE('brawler') && !hasE('improved brawler')) addMod('fighting', 'Brawler (nat. weapon tag)', 0);
  
  // ── Combat — Shooting ───────────────────────────────────
  if (hasE('steady hands'))  addMod('shooting',    'Steady Hands (no running penalty)', 0);
  if (hasE('marksman'))      addMod('shooting',    'Marksman (+2 if no move)', 2);
  if (hasE('dead shot'))     addMod('shooting',    'Dead Shot (dbl damage raise)', 0);
  
  // ── Social ──────────────────────────────────────────────
  if (hasE('very attractive')) {
    addMod('performance', 'Very Attractive (+2)', 2);
    addMod('persuasion', 'Very Attractive (+2)', 2);
  } else if (hasE('attractive')) {
    addMod('performance', 'Attractive (+1)', 1);
    addMod('persuasion', 'Attractive (+1)', 1);
  }
  if (hasE('charismatic'))   addMod('persuasion',  'Charismatic (+2)', 2);
  if (hasE('streetwise'))    addMod('persuasion',  'Streetwise (+2 criminal)', 2);
  if (hasE('strong willed')) { addMod('intimidation','Strong Willed (+2)', 2); addMod('taunt','Strong Willed (+2)', 2); }
  
  // ── Knowledge / Research ────────────────────────────────
  if (hasE('investigator'))  { addMod('research',  'Investigator (+2)', 2); addMod('notice','Investigator (+2 research)', 2); }
  if (hasE('scholar'))       { addMod('academics', 'Scholar (+2)', 2); addMod('science','Scholar (+2)', 2); }
  if (hasE('jack-of-all'))   addMod('notice',      'Jack-of-All (no unskilled penalty)', 0);
  
  // ── Vehicles ────────────────────────────────────────────
  if (hasE('ace'))           { addMod('driving',   'Ace (+2)', 2); addMod('piloting','Ace (+2)', 2); addMod('boating','Ace (+2)', 2); }
  
  // ── Survival / Stealth ──────────────────────────────────
  if (hasE('woodsman'))      { addMod('survival',  'Woodsman (+2)', 2); addMod('stealth','Woodsman (+2 wild)', 2); }
  if (hasE('thief'))         { addMod('stealth',   'Thief (+2)', 2); addMod('thievery','Thief (+2)', 2); }
  
  // ── Healing ─────────────────────────────────────────────
  if (hasE('healer'))        addMod('healing',     'Healer (+2)', 2);
  
  const liqCourage = allActiveEdges.find(e => e.nm.includes('liquid courage'));
  if (liqCourage && liqCourage.eff.includes('vigor')) {
    addMod('vigor', 'Liquid Courage (+1 die type)', 0);
  }

  // Active powers with skill effects
  state.powers.filter(p=>p.active).forEach(p=>{
    const nm = (p.name||'').toLowerCase();
    if (nm.includes('awareness')) addMod('notice', `Awareness (ignores ${parseInt(p.base)||1} attack penalties)`, 0);
    if (nm.includes('genius')) addMod('com. knowledge', 'Genius (free reroll)', 0);
  });
  return mods;
}
